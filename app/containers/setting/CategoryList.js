...
const height = Dimensions.get('window').height    // 物理尺寸
  ...
  _renderItem = item => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          let topHeight = (width === 375 && height === 812 ? 88 : 64) + 44
          if ((CateData.data.length - index + 1) * 45 >= (height - topHeight) )
            this._flatList.scrollToOffset({animated: true, offset: (index * 45 > (height - topHeight ) ? (height - topHeight) : index * 45)})
          this._sectionList.scrollToLocation({itemIndex: 0, sectionIndex: 0, animated: false, viewOffset: 0})
